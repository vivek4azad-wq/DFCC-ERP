...
// 1. Read db.js
const dbPath = '/Users/vivekazad/.gemini/antigravity/scratch/antigravity-ims/js/db.js';
const dbContent = fs.readFileSync(dbPath, 'utf8');

const pcMatch = dbContent.match(/const pc=\[([\s\S]*?)\];/);
const cvMatch = dbContent.match(/const cv=\[([\s\S]*?)\];/);
const pcRaw = eval('[' + pcMatch[1] + ']');
const cvRaw = eval('[' + cvMatch[1] + ']');

// Waypoints for GPS calculation
const mainWaypoints = [
  { km: 1167.210, lat: 30.3200, lon: 76.7350 },
  { km: 1170.435, lat: 30.3442, lon: 76.7121 },
  { km: 1188.575, lat: 30.4318, lon: 76.5184 },
  { km: 1202.015, lat: 30.6302, lon: 76.3881 },
  { km: 1213.187, lat: 30.6651, lon: 76.2974 },
  { km: 1229.087, lat: 30.7063, lon: 76.2205 },
  { km: 1235.837, lat: 30.7601, lon: 76.1052 },
  { km: 1249.720, lat: 30.8524, lon: 75.9802 }
];

function getCoords(km, isLink = false) {
  if (isLink) {
    const t = Math.max(0, Math.min(1, (km - 1168.697) / (1178.150 - 1168.697)));
    const lat = 30.3442 + t * (30.4841 - 30.3442);
    const lon = 76.7121 + t * (76.5950 - 76.7121);
    return { latitude: Number(lat.toFixed(6)), longitude: Number(lon.toFixed(6)) };
  }
  for (let i = 0; i < mainWaypoints.length - 1; i++) {
    const w1 = mainWaypoints[i];
    const w2 = mainWaypoints[i + 1];
  ...
  if (km < mainWaypoints[0].km) return { latitude: mainWaypoints[0].lat, longitude: mainWaypoints[0].lon };
  return { latitude: mainWaypoints[mainWaypoints.length - 1].lat, longitude: mainWaypoints[mainWaypoints.length - 1].lon };
}

let doc = `# Phase 0 Survey Report: Rail Diary (DFCCIL IMSD SMUN Unit ERP)
...
// Append all 161 PC rows
pcRaw.forEach(r => {
  doc += `| ${r[0]} | ${r[1]} | ${r[2]} | ${r[3]} | ${r[4]} | ${r[5]} | ${r[6]} | ${r[7]} | ${r[8]} | ${r[9] || '-'} |\n`;
});
...
// Append all 95 Curve rows
cvRaw.forEach(r => {
  doc += `| ${r[0]} | ${r[1]} | ${r[2]} | ${r[3]} | ${r[4]} | ${r[5]} | ${r[7]} | ${r[8]} | ${r[9]} | ${r[10]} | ${r[12]} | ${r[14] || '-'} | ${r[15] || '-'} |\n`;
});
...
const outputPath = '/Users/vivekazad/.gemini/antigravity/scratch/rail_diary/.agents/teamwork_preview_spec_miner_survey_1/survey_report.md';
fs.writeFileSync(outputPath, doc, 'utf8');
console.log('Successfully generated survey_report.md at:', outputPath);
