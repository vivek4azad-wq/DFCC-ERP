  ...
  return (
    <section className="space-y-4 animate-fadeIn">
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden">
        <div className="overflow-x-auto"><table className="w-full min-w-[1120px] text-left text-xs"><thead className="bg-[#e8f1fb] dark:bg-slate-800 text-[#0f2b5c] dark:text-slate-200 uppercase text-[10px]"><tr><th className="p-3">S.No</th><th className="p-3">Item Code (SAP)</th><th className="p-3">Item Name</th><th className="p-3">Item Type</th><th className="p-3 text-right">Receipt</th><th className="p-3 text-right">Transfer</th><th className="p-3 text-right">Issue</th><th className="p-3 text-right">Balance</th><th className="p-3">SAP Description</th><th className="p-3">Ledger</th></tr></thead><tbody className="divide-y divide-slate-100 dark:divide-slate-800">{filteredItems.map((item, index) => {
          const partial = item.matchStatus.startsWith('SAP ID partial');
          return <tr key={ledgerKey(item)} className="hover:bg-slate-50 dark:hover:bg-slate-800/60"><td className="p-3 text-right text-slate-500">{index + 1}</td><td className="p-3 font-mono font-black text-blue-700 dark:text-cyan-300">{item.sapMaterial || <span className="text-amber-700 dark:text-amber-300">Pending</span>}{partial && <div className="text-[9px] text-amber-700 dark:text-amber-300 mt-1">Partial — verify</div>}</td><td className="p-3"><div className="font-bold text-slate-900 dark:text-white">{item.itemName}</div><div className="text-[10px] text-slate-500 mt-1">Ledger page: {item.ledgerPage}</div></td><td className="p-3"><span className="rounded-full bg-indigo-50 dark:bg-indigo-950/60 border border-indigo-100 dark:border-indigo-800 px-2 py-1 text-[10px] font-bold text-indigo-700 dark:text-indigo-300">{itemType(item.source)}</span></td><td className="p-3 text-right font-mono">{fmt(item.totalReceipt)}</td><td className="p-3 text-right font-mono">{fmt(item.totalTransfer)}</td><td className="p-3 text-right font-mono">{fmt(item.totalIssue)}</td><td className="p-3 text-right font-mono font-bold">{fmt(item.closingBalance)}</td><td className="p-3 max-w-sm"><div className="font-medium text-slate-700 dark:text-slate-300">{item.sapDescription || <span className="text-amber-700 dark:text-amber-300">SAP description pending verification</span>}</div><div className="text-[10px] text-slate-500 mt-1">{item.sapUom}{partial ? ' • partial description match' : ''}</div></td><td className="p-3"><button onClick={() => setSelectedItem(item)} className="inline-flex items-center gap-1 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white px-2.5 py-1.5 font-bold"><BookOpen className="w-3.5 h-3.5" /> View <ChevronRight className="w-3.5 h-3.5" /></button></td></tr>;
        })}</tbody></table></div>
